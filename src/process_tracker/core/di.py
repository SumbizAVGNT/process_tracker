...
# ---------- Конкретные провайдеры сервисов ----------

@asynccontextmanager
async def provide_task_service() -> AsyncIterator[TaskService]:
    async with provide(lambda s: TaskService(s)) as svc:
        yield svc


@asynccontextmanager
async def provide_process_service() -> AsyncIterator[ProcessService]:
    async with provide(lambda s: ProcessService(s)) as svc:
        yield svc


# ---------- FastAPI dependency (на будущее/для роутов) ----------
